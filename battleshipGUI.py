...
class GUI:
    def __init__(self) -> None:
        ...
        self.root = Tk()
        self.root.title("Battle Ship by Anh Duy Tran and ")
        ...
        self.buttons = [[NewButton(self, self.root, i, j) for i in range(10)] for j in range(10)]
        for i in range(10):
            for j in range(10):
        ...
        self.__editButton = EditButton(self, self.root)
        ...
        self.beginShapeButton = BeginShapeButton(self, self.root)
...
class NewButton:
    ...
    """
    def __init__(self, GUI: GUI, win, x, y) -> None:
        self.button = Button(win, font=Font(size=15), command=self.pressed)
    ...
    def pressed(self):
        ...
        if self.GUI.Editing and not self.GUI.beginNewShape:
            self.GUI.pushMessage("press \"Begin Shape\" to start selecting", "red")
            return

        if self.GUI.Editing and self.GUI.beginNewShape:
            self.GUI.pushMessage("")
            
            if self.GUI.shipName.get() == "":
                self.GUI.pushMessage("New ship needs name!", "red")
                return
            if self.button["text"] != "":
                #unselect the tile
                if [self.x, self.y] in self.GUI.newShape["coord"]:
                    self.GUI.newShape["coord"].remove([self.x, self.y])
                    self.button["text"] = ""
                    return
                
                self.GUI.pushMessage("Location already occupied!", "red")
                return

            #check if the newly selected ship part is valid or not 
            # (have an adjacent side to another of its part)
            valid = True
            if len(self.GUI.newShape["coord"]) != 0:
                valid = False
                if [self.x + 1, self.y] in self.GUI.newShape["coord"]:
                    valid = True
                elif [self.x - 1, self.y] in self.GUI.newShape["coord"]:
                    valid = True
                elif [self.x, self.y + 1] in self.GUI.newShape["coord"]:
                    valid = True
                elif [self.x, self.y - 1] in self.GUI.newShape["coord"]:
                    valid = True

            if not valid:
                self.GUI.pushMessage("new ship part need to be close together!")
                return

            #edit the board content according the user input
            self.button["text"] = self.GUI.shipName.get()[0].upper()
            self.button["fg"] = self.GUI.randomColor

            # edit the new ship atributes
            self.GUI.newShape["coord"].append([self.x, self.y])
            return
        ...
        if self.GUI.gameOver:
            self.GUI.pushMessage("Game is over, \nload another game to continue playing")
            return
        elif not self.GUI.loaded:
            self.GUI.pushMessage("Game not loaded!")
            return

        self.GUI.pushMessage("")

        if self.GUI.BOARD[self.x][self.y] == -1:
            self.GUI.pushMessage("Location has already been shot at!")
            return

        if self.GUI.BOARD[self.x][self.y]:
            self.button["text"] = "X"
            self.button["fg"] = "red"
        else:
            self.button["text"] = "O"
        ...
        self.GUI.shoot(self.x, self.y)


class EditButton:
    def __init__(self, GUI: GUI, win) -> None:
        self.button = Button(win, text = "Edit Mode = OFF", fg = "red", command=self.pressed)
        self.GUI = GUI
        return

    def pressed(self):
        if self.GUI.beginNewShape:
            self.GUI.pushMessage("End shape to turn off Edit Mode!")
            return

        # change the state of the program 
        self.GUI.Editing = not self.GUI.Editing
        ...
        if self.GUI.Editing:
            ...
            self.GUI.shipNameLabel.place(x=600, y = 300)
            self.GUI.shipName.place(x=600, y=320)
            self.GUI.beginShapeButton.button.place(x=600, y=350)
        else:
            ...
            try:
                ...
                f.write("\n".join(self.GUI.newEditedBoard))
                f.close()
                self.GUI.pushMessage("Successfully created a new board!", "green")

            except IOError:
                self.GUI.pushMessage("Error while writing the gameboard!")
                return
            ...
            self.GUI.newEditedBoard = []

            # remove the editing button, label, entry element from the GUI
            self.GUI.shipNameLabel.place_forget()
            self.GUI.shipName.place_forget()
            self.GUI.beginShapeButton.button.place_forget()
        return
            

class BeginShapeButton:
    """
    A button to seperate the shape selected by the user when creating a new game board
    """
    def __init__(self, GUI: GUI, win) -> None:
        self.button = Button(win, text = "Begin Shape", command=self.pressed)
        self.GUI = GUI
        pass

    def pressed(self):
        #change the state of the program
        self.GUI.beginNewShape = not self.GUI.beginNewShape
        ...
        if self.GUI.beginNewShape:
            ...
            if self.GUI.shipName.get() == "":
                ...
                self.GUI.newShape["name"] = self.GUI.shipName.get()
                self.GUI.randomColor = random.choice(["red", "green", "blue", "yellow", "cyan", "purple", "brown"])
